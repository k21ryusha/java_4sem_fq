...
public class RaceSimulator {
    ...
    public double simulateRaceTime(TeamManager team, Car car, MainDriver driver, Track track, Weather weather) {
        ...
        double pitStopSeconds = 16.0;
        if (car.getTyres().getDurability() < 70) {
            pitStopSeconds = 22.0;
        }
    ...
    private double calculatePlayerLapTime(TeamManager team, Car car, MainDriver driver, Track track, Weather weather) {
        double rainPerformanceWeight = 0.14;
        if (weather == Weather.DRY) {
            rainPerformanceWeight = 0.04;
        }
        ...
        double principalImpact = 0.55;
        if (team.getPrincipal() != null) {
            principalImpact = team.getPrincipal().getSkill() / 100.0;
        }
        double technicalDirectorImpact = 0.55;
        if (team.getTechnicalDirector() != null) {
            technicalDirectorImpact = team.getTechnicalDirector().getSkill() / 100.0;
        }
        ...
        double baseLapSeconds = 55.0 + track.getLapKm() * 7.2;
        double performanceWindow = Math.max(0.84, 1.0 - (totalPerformance - 0.72) * 0.35);
        double weatherPenalty = 1.0 + (1.0 - weather.getMultiplier()) * 0.35;
        ...
        return Math.max(62.0, Math.min(125.0, raceLap));
    }
    ...
    private double calculateBotLapTime(TeamManager bot, Track track, Weather weather) {
        ...
        double principalImpact = 0.58;
        if (bot.getPrincipal() != null) {
            principalImpact = bot.getPrincipal().getSkill() / 100.0;
        }
        double technicalDirectorImpact = 0.58;
        if (bot.getTechnicalDirector() != null) {
            technicalDirectorImpact = bot.getTechnicalDirector().getSkill() / 100.0;
        }
        ...
        double baseLapSeconds = 54.0 + track.getLapKm() * 6.8;
        double performanceWindow = Math.max(0.85, 1.0 - (totalPerformance - 0.76) * 0.32);
        double weatherPenalty = 1.0 + (1.0 - weather.getMultiplier()) * 0.35;
        ...
        return Math.max(62.0, Math.min(123.0, raceLap));
    }
}
